...
fn main(mut gba: agb::Gba) -> ! {
    ...
    // Center background and create color wheel
    ...
    loop {
        ...
        // Rotate colors by cycling through palette
        let rotation_offset = (frame_count / 4) % 13;
        ...
        let rotated_palette = agb::display::Palette16::new([
            color!(black), // 0: Always black
            original_colors[((0 + rotation_offset) % 13) as usize],
            original_colors[((1 + rotation_offset) % 13) as usize],
            original_colors[((2 + rotation_offset) % 13) as usize],
            original_colors[((3 + rotation_offset) % 13) as usize],
            original_colors[((4 + rotation_offset) % 13) as usize],
            original_colors[((5 + rotation_offset) % 13) as usize],
            original_colors[((6 + rotation_offset) % 13) as usize],
            original_colors[((7 + rotation_offset) % 13) as usize],
            original_colors[((8 + rotation_offset) % 13) as usize],
            original_colors[((9 + rotation_offset) % 13) as usize],
            original_colors[((10 + rotation_offset) % 13) as usize],
            original_colors[((11 + rotation_offset) % 13) as usize],
            original_colors[((12 + rotation_offset) % 13) as usize],
            color!(white), // 14: Always white center
            color!(gray),  // 15: Unused
        ]);

        agb::display::tiled::VRAM_MANAGER.set_background_palettes(&[rotated_palette]);
